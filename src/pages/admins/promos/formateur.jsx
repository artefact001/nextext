const AjouteFormateurPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Définition de l'endpoint en fonction du rôle sélectionné
    const endpoints = {
      Formateur: `${process.env.NEXT_PUBLIC_API_URL}/formateur/inscrire`,
      ChefDeProjet: `${process.env.NEXT_PUBLIC_API_URL}/chef-de-projet/inscrire`,
      Vigile: `${process.env.NEXT_PUBLIC_API_URL}/vigile/inscrire`
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        mutate(`${process.env.NEXT_PUBLIC_API_URL}/formateurs`); // Actualiser les formateurs
      } else {
        if (data.errors) {
  ...
  const filteredUsers = formateurs.filter(
    (user) => user.role === filterType
  );
  ...
  return (
    <Center display={'block'}>
      ...
      <SimpleGrid
        ...
        spacing={2}
      >
        <CardBox maxW={{ base: '100%', md: '100%', lg: '90%' }} maxH='70%'>
          <Heading justifyContent="center" mx="41%" as="h2" size="md" mb={4}>{role}</Heading>
          <Box mx="25%">
            <FormSelect
              id="role"
              name="role"
              ...
              options={[
                ...
                { value: 'Vigile', label: 'Vigile' }, // Ajout de Vigile
              ]}
              error={errors.role}
            />
          </Box>
          <form onSubmit={handleSubmit}>
            ...
            <Button mt={4} mx="25%" type="submit" alignItems='center' isLoading={isLoading} _hover={{ bg: "#110033" }} color="white" bg="#CE0033" width="50%" py={7}>
              Inscrire {role}
            </Button>
          </form>
